...
- **One call to action, exactly one.** One email drives ONE action. That action is EITHER a link OR a reply:
  - **Link CTA** — put the {{link}} token where the reader clicks: in the body at the CTA, in the `cta` line, or both (it's one merge token pointing to one URL, so repeating it is still one destination). Never put {{link}} in the preview or a subject line.
  - **Reply CTA** — the email asks the reader to reply (great for the early welcome emails); the `cta` starts with "Reply…" and there is NO {{link}} anywhere in the email.
  The `cta` field is that action line: it starts with a verb (get, claim, book, start, download, reply, read, watch, join, see, try, call, text, visit, save, send, grab, pick, choose, tell), names the specific thing the reader receives, and restates the benefit. Do not mix a link and a reply in the same email, and never add a second, competing destination.
