...
**D-063 · On-signup provisioning: an accepted intake auto-creates a working portal login (own-portal slice 4).**
Founder: "keep going." Wired the funnel to the platform. New: portal/accounts.ts — the login table (email → tenantId + SHA-256-hashed password, constant-time verify, persisted to JSON, swappable to a DB; in-memory when no file, for tests). portal/provision.ts::provisionTenant(store, accounts, dataDir, {email,name,intake}) — turns a signup into a working login: derives a stable tenant id from the email, generates the customer's brand brain (full S1–S9 + Soro cluster + keyword report + ad campaign, mock, £0) into their own run dir, creates the CRM tenant, issues a random temp password via the account store, records the first run; idempotent per email; if generation fails the login still works (artifacts deferred). buildPortalDeps now returns { portal, provision }. Server hook: AppDeps.onIntakeAccepted(intake, email) fired fire-and-forget from /api/intake after S0 accept (email from the Stripe order via orders.find, else an intake `_email` field) — provisioning never blocks or fails the intake, mirroring the marketing hooks. index.ts wires it to bundle.provision and logs the issued credentials (emailing them via the already-wired Postmark is the small next step). Proven two ways: 4 new unit tests (accounts create/verify/persist; provisionTenant issues a working login with a generated brand brain; idempotent per email) + a server-test that the intake hook fires with the order email and NOT on a rejected intake, AND a live HTTP end-to-end (fresh intake → background provisioning → the logged temp password logs the new client into their own dashboard showing their own generated cluster/keywords/pipeline). 275 tests, typecheck clean. Own-portal order: CRM core (D-060) ✓ → client auth + portal (D-061) ✓ → live run + brand-brain provisioning (D-062) ✓ → on-signup provisioning (this) ✓ → email the login + account connections → messaging rails → subscription billing.

**D-064 · The self-serve loop closes: a new signup gets emailed their login (own-portal slice 5).**
Founder: "keep going." Wired the login email so provisioning ends in the customer's inbox, not just a server log. New: portal/emails.ts::loginEmail() — a pure Postmark message builder ("Your Relaunch72 dashboard is ready": text + branded HTML, the portal link, their email + temp password, a change-password nudge; validates the recipient). provision.ts: ProvisionResult now carries `name`; PortalConfig gains onProvisioned(result) and bundle.provision calls it for NEW signups only (never on a repeat, never blocking provisioning). index.ts builds onProvisioned from POSTMARK_SERVER_TOKEN + EMAIL_FROM + publicBaseUrl/portal and sends via the already-wired makePostmark; with no token it's a graceful no-op (the temp password still logs). Transactional-only (Postmark, per the email hard rule). Proven: loginEmail builds a valid message carrying the password + portal link and refuses a bad recipient; buildPortalDeps fires onProvisioned exactly once for a new signup and not on a repeat. 278 tests, typecheck clean. The full self-serve loop is now: pay/intake → S0 accept → background brand-brain generation → tenant + login created → "dashboard ready" email → client logs into their own generated content. Own-portal order: CRM (D-060) ✓ → auth+portal (D-061) ✓ → live run (D-062) ✓ → on-signup provisioning (D-063) ✓ → login email (this) ✓ → account connections → messaging rails → subscription billing.

**D-065 · Subscription billing: the platform earns recurring revenue (own-portal slice 6). Pulled forward — it's what makes it a business.**
Founder: "yes do it." Chose to build subscription billing next (ahead of account connections/messaging in the D-064 order) because recurring revenue is what turns the product into a platform business. Built mock-first/DI-tested, live on a key — no rewrite. New: orchestrator/src/server/subscriptions.ts — the recurring layer, structurally decoupled from Stripe (maps StripeEvents, never imports the SDK): `Subscription` state (id, customer, email, plan, status, current_period_end), `subscriptionFromEvent()` mapping customer.subscription.created/updated/deleted + invoice.paid/payment_failed → a normalised record (older API `current_period_end` on the sub OR newer on the item; older invoice→sub link at top level OR newer under `parent.subscription_details`; unknown statuses normalise to `incomplete`, never crash), `planResolver()` (Stripe price id → internal plan key), `isActive()` (active/trialing gate the platform), and a store in two flavours — `memorySubscriptionStore` (£0 default) + `fileSubscriptionStore` (JSONL, last-write-per-sub-id) — both upsert-MERGE so a thin invoice event never wipes the plan/period learned from a richer subscription event, and both index by email (case-insensitive, newest wins) so the portal can gate a login. catalog.ts: `PLANS` (three recurring platform tiers, placeholder £-pricing the founder edits, never shown as a quote) + `provisionPlans`/`ensurePlanPrices` create recurring Stripe prices via a shared provisioner (one-off + recurring share `provisionItems`). stripe.ts: `createSubscriptionCheckout` (mode:'subscription', stamps plan+email onto subscription_data.metadata so webhooks resolve the account). app.ts: `POST /api/subscription` checkout route + the webhook now records subscription events when a store is wired (optional — orders still record without it). config.ts: `planIds` (STRIPE_PLAN_*) + `subscriptionsFile`. index.ts: file-backed store wired in, plan prices auto-provisioned on boot alongside the catalog. Proven: 23 new tests (event mapping across both API shapes, merge/email-index on both stores, isActive, subscription checkout params + unknown-plan reject, recurring-price provisioning idempotency, and app-level webhook-records-subscription + subscription-optional). 301 tests, typecheck clean. Nothing spends until a real key + a first subscriber; portal billing gating/display (show status, gate Run on an active sub — soft, so the £0 demo keeps working) is the natural next slice. Own-portal order: CRM (D-060) ✓ → auth+portal (D-061) ✓ → live run (D-062) ✓ → on-signup provisioning (D-063) ✓ → login email (D-064) ✓ → subscription billing (this) ✓ → portal billing gating + account connections → messaging rails → dashboards.

**D-066 · The portal billing screen: a client sees their plan, subscribes and manages it — soft-gated so the £0 demo still runs (own-portal slice 7).**
Founder: "yes please." Turned D-065's billing backend into a client-facing screen. New: portal/billing.ts — makeBilling(subscriptions, emailForTenant) resolves a logged-in tenant → its login email (accounts.findByTenant, new) → its subscription (findByEmail) into a `BillingView` (status, active?, planKey/planName, renewal date, customerId, and the purchasable plan options); planOptions()/planNameFor() format the PLANS catalog for display. portal/views.ts: a compact billing card on the dashboard aside (plan + status pill + link) and a full `billingPage` (current status + plan cards with Subscribe/Switch forms + a Manage-billing button). portal/router.ts: GET /portal/billing (the screen), POST /portal/subscribe (→ injected subscribeUrl → Stripe checkout redirect), POST /portal/manage (→ injected manageUrl → Stripe billing portal), and SOFT gating on POST /portal/run — only blocks when `billingEnforced` is on AND the sub isn't active, so by default (and for the demo) Run always fires. stripe.ts: createBillingPortalUrl (+ optional billingPortal on StripeLike). index.ts wires subscribeUrl→createSubscriptionCheckout, manageUrl→createBillingPortalUrl (both key-guarded — hidden at £0), billingEnforced from env (default off), and passes the subscription store into buildPortalDeps so the billing resolver lights up. Proven: 15 new tests (billing resolver active/none/no-email, planOptions, billingPage current-plan + manage-visibility + subscribe CTAs, and router-level: dashboard card, /portal/billing renders, 404 when off, subscribe redirects to Stripe, subscribe error path, manage opens the portal, run soft by default, run gated+blocked under enforcement, run allowed for an active sub) AND a live HTTP smoke (real server: demo tenant logs in → /portal/billing renders all three plans with prices + subscribe forms + "No plan" status; dashboard shows the billing card). 316 tests, typecheck clean. Nothing charges until a Stripe key + a real subscriber; enforcement stays off until we choose to gate. Own-portal order: CRM (D-060) ✓ → auth+portal (D-061) ✓ → live run (D-062) ✓ → on-signup provisioning (D-063) ✓ → login email (D-064) ✓ → subscription billing (D-065) ✓ → portal billing screen (this) ✓ → account connections → messaging rails → dashboards.
