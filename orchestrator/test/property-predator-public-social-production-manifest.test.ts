...
test('social launch additions preserve the existing services and add no database or deployment mutation', () => {
  ...
  assert.match(manifest, /name: property-predator-customer-email-live/);
  assert.match(manifest, /name: property-predator-twilio-sms-live\r?\n/);
  assert.match(manifest, /name: property-predator-twilio-sms-live-webhook/);
