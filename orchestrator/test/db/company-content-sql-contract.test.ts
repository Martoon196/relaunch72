...
test('0021 creates a hardened content command role without provider capability', async () => {
  const sql = normalise(await readFile(migrationUrl, 'utf8'));
  assert.match(sql, /CREATE ROLE r72_content_command LOGIN NOINHERIT/);
  assert.match(sql, /rolname = 'r72_content_command' AND rolcanlogin AND NOT rolinherit AND NOT rolsuper AND NOT rolcreatedb AND NOT rolcreaterole AND NOT rolreplication AND NOT rolbypassrls/);
  assert.match(sql, /REVOKE ALL ON ALL TABLES IN SCHEMA app FROM r72_content_command/);
  assert.match(sql, /REVOKE ALL ON ALL FUNCTIONS IN SCHEMA app_private FROM r72_content_command/);
  assert.match(sql, /REVOKE CREATE ON SCHEMA public FROM r72_content_command/);
...
test('0021 versions exact content provenance and computes content SHA-256 in PostgreSQL', async () => {
  ...
  assert.match(sql, /content_sha256 bytea GENERATED ALWAYS AS \( public\.digest\(pg_catalog\.convert_to\(content_body, 'UTF8'\), 'sha256'\) \) STORED/);
...
test('0021 carries a short-lived exact source attestation independent from approval', async () => {
  ...
  assert.match(sql, /company_content_source_attestations_command_insert/);
...
test('0021 forces workspace RLS, member reads, command writes and manager-only approval', async () => {
  const sql = normalise(await readFile(migrationUrl, 'utf8'));
  const tables = [
  ...
  assert.match(sql, /company_content_versions_command_insert/);
...
test('0021 makes versions and approval history append-only while reusing hashed receipts', async () => {
  const sql = normalise(await readFile(migrationUrl, 'utf8'));
  ...
  assert.match(sql, /command_name LIKE 'companyContent\.%'/);
