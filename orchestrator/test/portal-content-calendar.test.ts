...
function present(
  snapshot = createPropertyPredatorContentCalendarFixture(),
  filters: Readonly<{ mode?: unknown; date?: unknown; channel?: unknown }> = {},
) {
  return presentContentCalendar(snapshot, { ...OPTIONS, filters });
}
...
test('Content Calendar renders a premium accessible planner while every content control stays TEST-only and disabled', () => {
  ...
  assert.match(html, /<article class="ccal" aria-labelledby="ccal-title" data-provider-effects="none">/);
...
test('Content Calendar escapes hostile catalogue and planning labels', () => {
  ...
  const view = presentContentCalendar({ catalog: page([hostile]), slots: [hostileSlot] }, {
    ...OPTIONS,
