...
const JSON_MEDIA_TYPE = /^application\/(?:[a-z0-9!#$&^_.+-]+\+)?json(?:\s*;|$)/iu;
...
function clientId(raw: string): string {
  if (!SAFE_CLIENT_ID.test(raw)) throw transportError('client identity is invalid');
  return raw;
}
...
async function boundedBody(response: Response): Promise<string> {
  ...
  try {
    ...
    try { await reader.cancel(); } catch { /* Nothing else can be trusted or recovered. */ }
...
export function createPropertyPredatorCompanyAssetBridgeTransport(
  ...
  const endpoint = new URL('/api/internal/company-content/bridge', baseUrl);
