...
(function(){
  ...
  var SESSION_KEY = "relaunch72-checkout-session-v1";
  var ACCESS_KEY = "relaunch72-sandbox-access-v1";
  var ACCESS_PROMPTS = 0;
  var state = load();          // { answers:{}, step:0 }
  var app = document.getElementById("app");

  // Purchase context carried from the server-created Stripe Checkout success URL.
  var URLP = new URLSearchParams(location.search);
  var TIER = (URLP.get("tier") || "").toLowerCase();
  var URL_SESSION = URLP.get("session") || "";
  var SESSION = URL_SESSION;
  try {
    SESSION = URL_SESSION || sessionStorage.getItem(SESSION_KEY) || "";
    if(URL_SESSION) sessionStorage.setItem(SESSION_KEY,URL_SESSION);
  } catch(e) {}
  if(URL_SESSION){
    URLP.delete("session");
    try{ history.replaceState(null,"",location.pathname+(URLP.toString()?"?"+URLP.toString():"")+location.hash); }catch(e){}
  }
  var TIER_NAMES = { autopsy: "Marketing Autopsy", core: "Relaunch72 Core", pro: "Relaunch72 Pro" };
  if (TIER) { state.tier = TIER; save(); }

  function load(){ try{ var saved=JSON.parse(localStorage.getItem(KEY)) || {answers:{},step:0}; if("session" in saved){ delete saved.session; localStorage.setItem(KEY,JSON.stringify(saved)); } return saved; }catch(e){ return {answers:{},step:0}; } }
  ...
  function payload(){
    ...
    if (SESSION) out._stripe_session = SESSION;
    return out;
  }
  ...
  // Re-hydrate the form from a previously-downloaded intake JSON (the flat payload),
  // so a customer who got a downloaded copy never re-types 45 fields.
  function loadFromFile(file){
    var reader=new FileReader();
    reader.onload=function(){
      var parsed; try{ parsed=JSON.parse(reader.result); }catch(e){ parsed=null; }
      if(!parsed||typeof parsed!=="object"){ alert("That file doesn’t look like a Relaunch72 intake. Pick the .json the form downloaded for you."); return; }
      var loaded=0;
      FIELDS.forEach(function(f){ if(parsed[f.id]!=null && parsed[f.id]!==""){ state.answers[f.id]=parsed[f.id]; loaded++; } });
      if("consent" in parsed) state.answers.consent=!!parsed.consent;
      if(!loaded){ alert("Couldn’t find any answers in that file."); return; }
      save();
      renderReview();
    };
  ...
  function renderIntro(){
    app.innerHTML='<div class="wrap intro">'+
      ...
      '<div class="card">'+
        ...
        '<p class="foot" style="margin-top:14px">Already filled this in and got a downloaded copy? '+
          '<a href="#" id="resume-trigger" style="color:var(--electric);font-weight:600;text-decoration:none">Load it and skip to Send →</a>'+
          '<input type="file" id="resume" accept="application/json,.json" style="display:none"/></p>'+
      '</div>'+
      '<p class="foot">Private sandbox: acceptance tests the mock workflow only. It does not start a live delivery clock or customer fulfilment.</p>'+
    '</div>';
    document.getElementById("start").onclick=function(){ go(0); };
    var rf=document.getElementById("resume"), rt=document.getElementById("resume-trigger");
    if(rt&&rf){ rt.onclick=function(e){ e.preventDefault(); rf.click(); }; rf.onchange=function(){ if(rf.files&&rf.files[0]) loadFromFile(rf.files[0]); }; }
  }
  ...
  function renderReview(){
    ...
    if(allErrs.length){
      ...
      body='<p class="lede">Everything checks out. '+answeredCount()+' of '+FIELDS.length+' fields answered. Sending tests the private mock workflow; it does not start live customer work.</p>';
    }
    app.innerHTML='<div class="wrap"><div class="sec-head"><div class="n">Final step</div><h2>Review &amp; send</h2></div>'+
      body+
      '<label class="consent"><input type="checkbox" id="consent" '+(consentOk?'checked':'')+'/><span>I confirm my answers are accurate. This is a private test sandbox, so I will not include sensitive personal data. I understand the pack is AI-assisted and live privacy and fulfilment terms must be agreed before customer work.</span></label>'+
      '<div class="nav"><button class="btn ghost" id="back">← Back to sections</button>'+
        '<button class="btn" id="send" '+((allErrs.length||!consentOk)?'disabled':'')+'>Send test intake →</button></div>'+
      '<div class="foot">Sandbox acceptance creates mock workflow state only — no live delivery clock or customer fulfilment.</div></div>';
  ...
  function submit(){
    ...
    if(SPEC.submitEndpoint){
      ...
      try{ var access=sessionStorage.getItem(ACCESS_KEY)||""; if(access)headers["x-relaunch72-sandbox-token"]=access; }catch(e){}
      fetch(SPEC.submitEndpoint,{method:"POST",headers:headers,body:JSON.stringify(data)})
        .then(function(r){ return r.json().catch(function(){ return {}; }).then(function(body){ return {ok:r.ok,status:r.status,body:body}; }); })
        .then(function(result){
          var res=result.body||{};
          if(result.status===401&&ACCESS_PROMPTS<2){
            ACCESS_PROMPTS++;
            var entered=window.prompt("Private Relaunch72 test access code");
            if(entered&&entered.trim()){
              try{ sessionStorage.setItem(ACCESS_KEY,entered.trim()); }catch(e){}
              if(send){ send.disabled=false; }
              submit();
              return;
            }
          }
          if(result.ok&&res.accepted===true){
            try{ localStorage.removeItem(KEY); }catch(e){}
            try{ sessionStorage.removeItem(SESSION_KEY); }catch(e){}
            renderDone();
            return;
          }
          if(res.accepted===false&&Array.isArray(res.issues)&&res.issues.length){ renderRejected(res.issues); return; }
          renderSubmitError(data,res.error||("The server could not accept this intake (HTTP "+result.status+")."));
        })
        .catch(function(){ renderSubmitError(data,"We could not reach Relaunch72. Your answers are still saved in this browser."); });
    } else {
  ...
  function renderDone(){
    app.innerHTML='<div class="wrap done"><div class="tick">✓</div><h1>Sandbox test accepted.</h1>'+
      '<p class="lede">The private mock workflow has accepted this test intake. No live delivery clock, human fulfilment or customer email has started.</p>'+
  ...
  function renderSubmitError(data,message){
    app.innerHTML='<div class="wrap"><div class="sec-head"><div class="n">Not sent</div><h2>Your intake has not been accepted yet</h2></div>'+
      '<div class="nudge"><p>'+esc(message)+'</p></div>'+
      '<p class="lede">Nothing has been cleared. You can try again or download a backup copy.</p>'+
      '<div class="nav"><button class="btn ghost" id="savecopy">Download backup</button><button class="btn" id="retry">Try again →</button></div></div>';
    document.getElementById("savecopy").onclick=function(){ downloadCopy(data); };
    document.getElementById("retry").onclick=function(){ renderReview(); };
  }

  function downloadCopy(data){
    var name=(data.A1||"intake").toString().toLowerCase().replace(/[^a-z0-9]+/g,"-").replace(/^-|-$/g,"")||"intake";
    var backup={};
    FIELDS.forEach(function(f){ if(data[f.id]!=null) backup[f.id]=data[f.id]; });
    backup.consent=!!data.consent;
