 ...
 *
 *   npm run intake:build                       # → site/intake/index.html
 *   npm run intake:build -- --endpoint <url>   # also POST answers to a webhook
 *
...
function main(): void {
  ...
  const html = renderIntakeForm({ submitEndpoint: parseEndpoint(process.argv.slice(2)) });
