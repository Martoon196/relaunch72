...
function readiness(profile: PortalProductProfile): string {
  const railState = (state: PortalProductProfile['readinessRails'][number]['state']): { className: string; label: string } => {
    if (state === 'foundation') return { className: 'ready', label: 'Foundation' };
    if (state === 'reuse') return { className: 'reuse', label: 'Reuse existing' };
    return { className: '', label: 'Not connected' };
  };
