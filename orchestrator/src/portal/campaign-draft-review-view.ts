...
export function renderCampaignDraftPackReviewBody(
  ...
  const cards = results.map((result, index) => {
    ...
    const cta = ctaUrl
      ? `<a class="cdr-cta" href="${escapeHtml(ctaUrl)}">Review destination</a>`
      : '<span class="cdr-cta" aria-disabled="true">Destination unavailable</span>';
    return `<article aria-labelledby="cdr-pack-${index}"><header class="cdr-pack-head"><strong>${escapeHtml(result.draft.payload.platform)}</strong><span>Review required</span></header><h2 id="cdr-pack-${index}">${escapeHtml(result.draft.payload.title)}</h2><div class="cdr-body">${escapeHtml(result.draft.payload.body)}</div>${cta}<section class="cdr-card"><h2>Version proof</h2><dl><div><dt>Version</dt><dd>${escapeHtml(result.immutableSource.versionId)}</dd></div><div><dt>Content</dt><dd>${escapeHtml(short(result.immutableSource.contentSha256))}</dd></div><div><dt>Usage</dt><dd>${escapeHtml(short(result.immutableSource.usageSha256))}</dd></div></dl></section></article>`;
  }).join('');
