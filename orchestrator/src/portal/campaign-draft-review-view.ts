...
export function renderCampaignDraftPackReviewBody(
  ...
  const cards = results.map((result, index) => {
    ...
    const mediaMarkup = media
      ...
      : '';
    return `<article aria-labelledby="cdr-pack-${index}"><header class="cdr-pack-head"><strong>${escapeHtml(result.draft.payload.platform)}</strong><span>Review required</span></header>${mediaMarkup}<h2 id="cdr-pack-${index}">${escapeHtml(result.draft.payload.title)}</h2><div class="cdr-body">${escapeHtml(result.draft.payload.body)}</div>${cta}<section class="cdr-card"><h2>Version proof</h2><dl><div><dt>Version</dt><dd>${escapeHtml(result.immutableSource.versionId)}</dd></div><div><dt>Content</dt><dd>${escapeHtml(short(result.immutableSource.contentSha256))}</dd></div><div><dt>Usage</dt><dd>${escapeHtml(short(result.immutableSource.usageSha256))}</dd></div>${media ? `<div><dt>Media</dt><dd>${escapeHtml(short(media.contentSha256))}</dd></div>` : ''}</dl></section></article>`;
  }).join('');
