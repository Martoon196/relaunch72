...
export function renderContentControlRoomBody(
  ...
  return `${workspaceNavigation}<style data-property-predator-content-control>${CONTENT_CONTROL_ROOM_STYLE}</style><article class="ccr" aria-labelledby="ccr-title">
    ...
    <section class="ccr-truth" aria-label="Content safety boundary"><span class="ccr-truth-mark">Truth boundary</span><p><strong>Exact review is available; outbound remains separate.</strong> Open the immutable version to inspect and approve its complete bytes. No post, message, schedule or provider call happens from this catalogue.</p><span class="ccr-readonly">${view.canManage ? 'Exact review controls' : view.canWrite ? 'Submit controls' : 'Review control only'}</span></section>${notice(view.notice)}${ownedSeedProofAction}
