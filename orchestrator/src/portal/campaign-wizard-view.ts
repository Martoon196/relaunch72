...
function generationForm(
  ...
  return `<form class="cwiz-generate" method="post" action="${escapeHtml(action.actionUrl)}" data-campaign-review-draft-form data-provider-effects="generation-only" data-outbound-effects="none">${hidden('_csrf', action.csrfToken)}${hidden('command_key', action.commandKey)}${hidden('expected_plan_sha256', plan.planSha256)}${hidden('laps', plan.selection.key)}${hidden('provider_effects', 'generation_only')}<div class="cwiz-generate-head"><div><h3>Generate one real review draft</h3><p>The company-content model may run once. The exact result returns here for human review and cannot send, schedule or publish.</p></div><span class="cwiz-generate-badge">Ceiling ${action.maximumCostMinor.toLocaleString('en-GB')}</span></div><div class="cwiz-generate-grid"><label class="cwiz-field">Platform<select name="platform" required><option value="linkedin">LinkedIn</option><option value="instagram">Instagram</option><option value="facebook">Facebook</option><option value="x">X</option></select></label><label class="cwiz-field">Tone<select name="tone" required><option value="direct and useful">Direct and useful</option><option value="educational and evidence-led">Educational and evidence-led</option><option value="challenging but credible">Challenging but credible</option></select></label></div><label class="cwiz-field">Draft brief<textarea name="topic" maxlength="400" required placeholder="What should this exact draft teach, challenge or move forward?"></textarea><span>No customer, lead or affiliate-private information.</span></label><div class="cwiz-generate-grid"><div class="cwiz-generate-options"><strong>Exact approved fact version</strong>${factOptions}</div><div class="cwiz-generate-options"><strong>Exact approved asset version</strong>${assetOptions}</div></div><label class="cwiz-generate-confirm"><input type="checkbox" name="confirm_generation_only" value="confirmed" required><span>I authorise this one generation-only provider effect. I understand the result remains unapproved and has no outbound capability.</span></label><button class="cwiz-generate-submit" type="submit">Generate review draft</button></form>`;
}
