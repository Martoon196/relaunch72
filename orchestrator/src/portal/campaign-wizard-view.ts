...
function draftRecipe(plan: PropertyPredatorMarketingDraftPlan | undefined): string {
  if (!plan) return '';
  ...
  return `<section class="cwiz-brain" aria-labelledby="cwiz-brain-title" data-marketing-draft-preflight data-callable="false" data-persisted="false" data-provider-effects="none"><form class="cwiz-brain-control" method="get" action="${CAMPAIGN_WIZARD_ROUTE}"><span class="cwiz-kicker">Read-only marketing preflight</span><h2 id="cwiz-brain-title">Pick the conversion move</h2><p>Choose the canonical Property Predator journey milestone. Growth HQ will recalculate an inert recipe from registered hashes only.</p><label class="cwiz-brain-select">Journey and target milestone<select name="laps">${options}</select></label><button class="cwiz-brain-apply" type="submit">Refresh draft recipe</button></form><div class="cwiz-brain-route"><div class="cwiz-brain-top"><div><span class="cwiz-kicker">LAPS route · ${escapeHtml(plan.selection.journeyName)}</span><h2>${escapeHtml(from)} → ${escapeHtml(plan.selection.targetMilestoneName)}</h2></div><span class="cwiz-brain-state${plan.readiness === 'draft_recipe_ready' ? ' ready' : ''}">${plan.readiness === 'draft_recipe_ready' ? 'Draft recipe ready' : 'Draft recipe blocked'}</span></div><div class="cwiz-brain-stage"><strong>${escapeHtml(plan.selection.marketingJob)}</strong><span>Canonical journey v${plan.selection.journeyVersion.toLocaleString('en-GB')} · ${escapeHtml(plan.selection.journeyDefinitionSha256.slice(0, 12))}…</span></div><div class="cwiz-brain-grid"><section class="cwiz-brain-card"><h3>Adapted internal methods</h3><ul class="cwiz-brain-list">${methods}</ul></section><section class="cwiz-brain-card"><h3>Specialist handoff order</h3><ol class="cwiz-brain-list">${handoffs}</ol></section><section class="cwiz-brain-card"><h3>Why anything is blocked</h3><ul class="cwiz-brain-list">${blockers}</ul></section><section class="cwiz-brain-card"><h3>Immutable provenance</h3><div class="cwiz-brain-proof"><span>${packProof}</span><span>${brainProof}</span><span>Plan ${escapeHtml(plan.planSha256.slice(0, 12))}…</span></div></section></div><div class="cwiz-brain-boundary"><strong>Planning metadata only.</strong> Private training bodies are not loaded. No model is called, nothing is persisted by the campaign command, and provider effects remain NONE.</div></div></section>`;
}
