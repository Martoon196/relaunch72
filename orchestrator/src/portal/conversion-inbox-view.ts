...
export function renderConversionInboxBody(
  ...
  return `<section class="ci" data-property-predator-conversion-inbox data-environment="${hasLive ? 'controlled' : 'test'}">
    ...
    <header class="ci-head"><div><span class="ci-kicker">Growth HQ · Conversion Inbox</span><h1>Every channel. <em>One human queue.</em></h1><p>Turn engagement into confident conversations with the lead journey, consent and exact approval state visible beside every reply. <a href="/portal/inbox/social" style="color:#00e5cc;font-weight:800">Open live social messages →</a></p></div><div class="ci-mode"><strong>${escapeHtml(mode)}</strong><span>${escapeHtml(modeDetail)}</span></div></header>
