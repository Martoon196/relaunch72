...
const CONVERSION_INBOX_STYLE = `
  ...
  .ci{--ci-bg:var(--canvas);--ci-panel:var(--panel);--ci-raised:var(--panel-subtle);--ci-line:var(--line);--ci-line-strong:var(--line-strong);--ci-ink:var(--ink);--ci-muted:var(--muted);--ci-faint:var(--faint);--ci-teal:var(--accent-deep);--ci-red:var(--danger);--ci-green:var(--success);border-color:var(--ci-line)}
  .ci-head{grid-template-columns:minmax(360px,1fr) minmax(220px,320px);align-items:center;background:linear-gradient(135deg,var(--panel),var(--panel-subtle))}.ci-head>div{min-width:0}.ci-head h1{overflow-wrap:normal;word-break:normal;hyphens:none}.ci-head p{max-width:680px}.ci-mode{min-width:0;background:var(--accent-soft);border-color:var(--line-strong)}.ci-mode strong{color:var(--accent-deep)}.ci-mode span{color:var(--muted)}
  .ci-truth,.ci-toolbar,.ci-channels,.ci-thread,.ci-empty-thread{background:var(--panel-subtle)}.ci-queue-head,.ci-thread-head,.ci-composer{background:var(--panel)}.ci-channel[aria-current="page"],.ci-conversation>a[aria-current="true"],.ci-test-stamp,.ci-inbound-proof{background:var(--accent-soft);border-color:var(--accent);color:var(--accent-deep)}.ci-channel-count,.ci-avatar,.ci-stat,.ci-consent,.ci-step,.ci-delivery-card,.ci-composer textarea{background:var(--panel)}.ci-conversation>a:hover,.ci-conversation>a:focus-visible{background:var(--panel-subtle)}.ci-preview,.ci-message-body{color:var(--ink)}.ci-message[data-direction="outbound"] .ci-message-body{background:var(--accent-soft);border-color:var(--accent)}.ci-message[data-direction="internal_note"] .ci-message-body,.ci-notice[data-kind="info"]{background:var(--info-soft)}.ci-rail-activity[data-rail-state="attention"],.ci-notice[data-kind="error"]{background:var(--danger-soft)}.ci-notice{background:var(--accent-soft)}
  @media(max-width:1100px){.ci-head{grid-template-columns:1fr;align-items:start}.ci-mode{width:100%;max-width:none}}
...
export function renderConversionInboxBody(
  ...
  const modeDetail = hasLive
    ? hasLinkedIn
      ? 'Signed office replies may be live. LinkedIn projections stay read-only.'
      : 'Only signed office replies may be live. Other loaded channels are simulated.'
  ...
  return `<section class="ci" data-property-predator-conversion-inbox data-environment="${hasLive ? 'controlled' : 'test'}">
    ...
    <header class="ci-head"><div><span class="ci-kicker">Growth HQ · Inbox</span><h1>Conversations that <em>need you.</em></h1><p>Work messages and follow-ups with consent and review state beside each reply. Live social is shown in its own source.</p></div><div class="ci-mode"><strong>${escapeHtml(mode)}</strong><span>${escapeHtml(modeDetail)}</span></div></header>
    <div class="ci-truth"><strong>${escapeHtml(truthTitle)}</strong><p>${escapeHtml(truthDetail)}</p><span class="ci-snapshot">${escapeHtml(view.workspaceName)} · Snapshot ${dateTime(view.asOf, view.timezone)}</span></div>${renderNotice(view)}${options.sourceStatuses ? renderInboxSourceSwitcher(options.sourceStatuses, 'conversion') : ''}
