...
function renderTranscript(thread: ConversionInboxSelectedThreadView, timezone: string): string {
  ...
  const items = thread.messages.map((message) => `<li class="ci-message" data-direction="${escapeHtml(message.direction)}">
    <div class="ci-message-head"><strong>${escapeHtml(message.authorLabel)}</strong><span>·</span>${dateTime(message.occurredAt, timezone, true)}</div>
    ...
    ${message.deliveryLabel ? `<div class="ci-delivery">${escapeHtml(message.deliveryLabel)} · no real delivery occurred</div>` : ''}
  </li>`).join('');
