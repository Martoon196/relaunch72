...
import { renderInboxSourceSwitcher } from './inbox-source-switcher-view.js';
...
.zmsg{--z-panel:var(--panel);--z-subtle:var(--panel-subtle);--z-ink:var(--ink);--z-muted:var(--muted);--z-line:var(--line);--z-line-strong:var(--line-strong);--z-accent:var(--accent-deep);color:var(--z-ink)}.zmsg-head{display:grid;grid-template-columns:minmax(360px,1fr) minmax(220px,320px);align-items:center}.zmsg-head>div{min-width:0}.zmsg-head h1{overflow-wrap:normal;word-break:normal;hyphens:none}.zmsg-kicker,.zmsg-kind{color:var(--z-accent)}.zmsg-sub,.zmsg-preview,.zmsg-meta,.zmsg-note,.zmsg-empty{color:var(--z-muted)}.zmsg-badge{justify-self:end;max-width:320px;border-color:var(--z-line-strong);background:var(--accent-soft);color:var(--z-accent);white-space:normal}.zmsg-tabs a{border-color:var(--z-line);color:var(--z-ink)}.zmsg-tabs a[aria-current]{border-color:var(--accent);color:var(--z-accent)}.zmsg-panel{background:var(--z-panel);border-color:var(--z-line)}.zmsg-panel h2,.zmsg-thread-head,.zmsg-compose{border-color:var(--z-line)}.zmsg-list a{border-color:var(--z-line);color:var(--z-ink)}.zmsg-list a[aria-current],.zmsg-comment a[aria-current]{background:var(--accent-soft)}.zmsg-count,.zmsg-compose button,.zmsg-action{background:var(--accent);color:var(--accent-ink)}.zmsg-bubble,.zmsg-comment a{background:var(--z-subtle);border-color:var(--z-line);color:var(--z-ink)}.zmsg-bubble.out,.zmsg-comment.owner a,.zmsg-draft,.zmsg-notice{background:var(--accent-soft);border-color:var(--accent);color:var(--z-ink)}.zmsg-compose textarea{background:var(--z-subtle);border-color:var(--z-line-strong);color:var(--z-ink)}.zmsg-draft pre,.zmsg-check,.zmsg-notice p{color:var(--z-ink)}.zmsg-action.danger,.zmsg-notice.error{background:var(--danger-soft);color:var(--danger);border-color:var(--danger)}.zmsg-action.send{background:var(--success);color:var(--accent-ink)}.zmsg-readonly{background:var(--warning-soft);border-color:var(--warning);color:var(--ink)}
@media(max-width:1100px){.zmsg-head{grid-template-columns:1fr;align-items:start}.zmsg-badge{justify-self:start;max-width:none}}
.zmsg-readonly{background:var(--info-soft);border-color:var(--line-strong);color:var(--ink)}
</style>`;
}
...
export interface ZernioMessagingViewOptions {
  ...
  readonly sourceStatuses?: readonly InboxSourceStatus[];
}
...
export function renderZernioMessagingBody(
  ...
  if (!snapshot.ok) {
    return `${style()}<section class="zmsg">${options.sourceStatuses ? renderInboxSourceSwitcher(options.sourceStatuses, 'social') : ''}<div class="zmsg-panel zmsg-empty"><h1>${snapshot.kind === 'not_connected' ? 'Connect a social account' : snapshot.kind === 'forbidden' || snapshot.kind === 'unauthenticated' ? 'Social message permission required' : 'Social messages could not be refreshed'}</h1><p>${snapshot.kind === 'not_connected' ? 'No active social account is connected. Other inbox sources remain available.' : snapshot.kind === 'forbidden' || snapshot.kind === 'unauthenticated' ? 'This account cannot read the live social source.' : 'The live source did not return a confirmed result. No provider action ran; other inbox sources remain available.'}</p></div></section>`;
  }
  const sourceSwitcher = options.sourceStatuses
    ? renderInboxSourceSwitcher(options.sourceStatuses, 'social') : '';
  const notice = sourceSwitcher + (options.notice
  ...
  return `${style()}<section class="zmsg">${notice}<header class="zmsg-head"><div><div class="zmsg-kicker">Growth HQ · Live social inbox</div><h1>Social messages &amp; comments</h1><p class="zmsg-sub">Read DMs and organisation comments from connected social accounts. Replies keep the existing draft, approval and send controls.</p></div><span class="zmsg-badge">LIVE READ · ${outboundStatus}</span></header><nav class="zmsg-tabs"><a href="/portal/inbox">Email &amp; rails</a><a href="${ZERNIO_MESSAGING_ROUTE}" aria-current="page">Social messages</a></nav><div class="zmsg-grid"><section class="zmsg-panel"><h2>DMs &amp; comments · ${snapshot.conversations.length + snapshot.commentPosts.length}${snapshot.queueTruncated ? '+' : ''}</h2>${queue ? `<ul class="zmsg-list">${queue}</ul>` : '<div class="zmsg-empty">No social conversations or commented posts are available yet.</div>'}</section><section class="zmsg-panel zmsg-thread">${thread}</section></div></section>`;
}
