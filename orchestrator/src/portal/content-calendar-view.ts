...
const CHANNEL_CLASS: Readonly<Record<ContentCalendarChannel, string>> = Object.freeze({
  linkedin: 'linkedin', instagram: 'instagram', facebook: 'facebook', email: 'email', webinar: 'webinar',
});
...
const CONTENT_CALENDAR_STYLE = `
  ...
  .ccal-slot{position:relative;min-width:0;border:1px solid var(--cal-line2);border-left:3px solid var(--cal-teal);border-radius:7px;background:var(--cal-raised);overflow:hidden}.ccal-slot.locked{border-left-color:var(--cal-amber)}.ccal-slot-top{display:flex;align-items:center;justify-content:space-between;gap:7px;padding:8px 9px;border-bottom:1px solid var(--cal-line)}.ccal-channel{display:inline-flex;align-items:center;gap:6px;min-width:0;color:var(--cal-muted);font-size:12px;font-weight:850}.ccal-channel-code{width:23px;height:23px;display:grid;place-items:center;border:1px solid currentColor;border-radius:5px;color:var(--cal-blue);font:900 10px var(--mono,monospace);text-transform:uppercase}.ccal-channel-code.instagram{color:#e38ad9}.ccal-channel-code.facebook{color:#91aeff}.ccal-channel-code.email{color:#e6c477}.ccal-channel-code.webinar{color:#bb92ff}.ccal-time{color:var(--cal-ink);font:900 12px var(--mono,monospace)}.ccal-slot-body{padding:9px}.ccal-slot-state{display:flex;gap:5px;flex-wrap:wrap;margin-bottom:7px}.ccal-chip{display:inline-flex;align-items:center;min-height:22px;border:1px solid var(--cal-line2);border-radius:999px;padding:2px 6px;color:var(--cal-faint);font:800 10px var(--mono,monospace);letter-spacing:.03em;text-transform:uppercase}.ccal-chip.test{border-color:#2d756d;background:var(--cal-teal-soft);color:var(--cal-teal)}.ccal-slot h3{margin:0;font-size:12px;line-height:1.4}.ccal-variant{margin:5px 0 0;color:var(--cal-muted);font-size:11px;line-height:1.4}.ccal-gate{display:flex;align-items:center;gap:6px;margin-top:8px;color:var(--cal-teal);font-size:11px;font-weight:900}.ccal-gate::before{content:"";width:6px;height:6px;border-radius:50%;background:currentColor}.ccal-slot.locked .ccal-gate{color:var(--cal-amber)}.ccal-slot details{border-top:1px solid var(--cal-line)}.ccal-slot summary{min-height:40px;display:flex;align-items:center;justify-content:space-between;gap:7px;list-style:none;padding:0 9px;color:var(--cal-faint);font:800 10px var(--mono,monospace);cursor:pointer;text-transform:uppercase}.ccal-slot summary::-webkit-details-marker{display:none}.ccal-slot summary::after{content:"+";color:var(--cal-teal);font-size:15px}.ccal-slot details[open] summary::after{content:"−"}.ccal-proof{display:grid;gap:6px;padding:0 9px 9px}.ccal-proof-row{display:grid;grid-template-columns:55px minmax(0,1fr);gap:5px;color:var(--cal-muted);font-size:10px;line-height:1.35}.ccal-proof-row span{color:var(--cal-faint)}.ccal-proof-row code{color:var(--cal-ink)}.ccal-proof-note{margin:2px 0 0;color:var(--cal-muted);font-size:10px;line-height:1.45}
  .ccal-grid.month .ccal-day{min-height:160px}.ccal-grid.month .ccal-day-head{padding:7px 8px}.ccal-grid.month .ccal-day-head span{display:none}.ccal-grid.month .ccal-day-slots{gap:4px;padding:5px}.ccal-grid.month .ccal-slot-top{padding:5px 6px}.ccal-grid.month .ccal-channel span:last-child,.ccal-grid.month .ccal-slot-state,.ccal-grid.month .ccal-variant,.ccal-grid.month .ccal-slot details{display:none}.ccal-grid.month .ccal-slot-body{padding:6px}.ccal-grid.month .ccal-slot h3{display:-webkit-box;overflow:hidden;-webkit-box-orient:vertical;-webkit-line-clamp:2}.ccal-grid.month .ccal-gate{margin-top:5px}.ccal-grid.month .ccal-empty-day{min-height:55px;border:0}
...
function plannerHref(view: ContentCalendarView, input: Readonly<{
  ...
  return `${CONTENT_CALENDAR_ROUTE}?view=${encodeURIComponent(mode)}&date=${encodeURIComponent(date)}&channel=${encodeURIComponent(channel)}`;
}

function slotCard(slot: ContentCalendarSlotView): string {
  const stateClass = slot.simulationEligible ? 'ready' : 'locked';
  return `<article class="ccal-slot ${stateClass}" id="${escapeHtml(slot.anchorId)}" aria-labelledby="${escapeHtml(slot.anchorId)}-title">
    ...
    <div class="ccal-slot-body"><div class="ccal-slot-state"><span class="ccal-chip test">TEST · ${escapeHtml(slot.plannerStateLabel)}</span>${slot.versionNumber === null ? '' : `<span class="ccal-chip">Immutable v${safeCount(slot.versionNumber)}</span>`}</div><h3 id="${escapeHtml(slot.anchorId)}-title">${escapeHtml(slot.title)}</h3><p class="ccal-variant">${escapeHtml(slot.variantLabel)}</p><span class="ccal-gate">${escapeHtml(slot.gateLabel)}</span></div>
    <details><summary>Planning proof</summary><div class="ccal-proof"><div class="ccal-proof-row"><span>Version</span><code>${escapeHtml(slot.contentVersionId)}</code></div><div class="ccal-proof-row"><span>Hash</span><code>${escapeHtml(slot.shortHash)}…</code></div><div class="ccal-proof-row"><span>Approval</span><strong>${escapeHtml(slot.approvalLabel)}</strong></div><div class="ccal-proof-row"><span>Source</span><strong>${escapeHtml(slot.sourceFreshnessLabel)}</strong></div><div class="ccal-proof-row"><span>Owner</span><strong>${escapeHtml(slot.ownerLabel)}</strong></div><div class="ccal-proof-row"><span>Goal</span><strong>${escapeHtml(slot.objectiveLabel)}</strong></div><p class="ccal-proof-note">${escapeHtml(slot.gateDetail)}</p><p class="ccal-proof-note"><strong>Draft/simulated only.</strong> This planner cannot call ${escapeHtml(slot.channelLabel)} or any provider.</p></div></details>
  </article>`;
}

function dayColumn(day: ContentCalendarDayView, mode: ContentCalendarMode): string {
  const slots = day.slots.map(slotCard).join('');
  return `<section class="ccal-day${day.isToday ? ' today' : ''}${day.inPrimaryPeriod ? '' : ' outside'}" aria-label="${escapeHtml(day.fullDateLabel)}"><header class="ccal-day-head"><strong>${escapeHtml(day.dayNumber)}</strong><span>${day.isToday ? 'Today · TEST' : escapeHtml(day.weekdayLabel)}</span></header><div class="ccal-day-slots">${slots || `<div class="ccal-empty-day"><span>${mode === 'month' ? '—' : 'No draft slots'}</span></div>`}</div></section>`;
}
...
export function renderContentCalendarBody(view: ContentCalendarView): string {
  const weekdays = view.days.slice(0, 7).map((day) => `<div class="ccal-weekday">${escapeHtml(day.weekdayLabel)}</div>`).join('');
  const days = view.days.map((day) => dayColumn(day, view.filters.mode)).join('');
  const warning = view.hasUnknownVersion
    ...
    : '';
  const bounded = view.inputTruncated
    ? ' Bounded safety limits were applied to the supplied catalogue or draft slots.'
    : '';
  return `<style data-property-predator-content-calendar>${CONTENT_CALENDAR_STYLE}</style><article class="ccal" aria-labelledby="ccal-title" data-provider-effects="none">
    ...
    <section class="ccal-safety" aria-label="Planner truth boundary"><span class="ccal-safety-mark">Truth boundary</span><p><strong>A slot is not a scheduled provider job.</strong> “Simulation ready” means only that exact version, approval and source provenance agree. Every content action shown is a disabled draft/simulated control.</p><span class="ccal-safety-badge">No provider calls</span></section>
    ...
    ${channelNav(view)}
    <section class="ccal-metrics" aria-label="Planner summary"><div class="ccal-metric"><small>Draft placements</small><strong>${safeCount(view.metrics.plannedSlots)}</strong><span>No provider jobs created</span></div><div class="ccal-metric ready"><small>Simulation ready</small><strong>${safeCount(view.metrics.simulationReady)}</strong><span>Exact gates agree</span></div><div class="ccal-metric blocked"><small>Gate locked</small><strong>${safeCount(view.metrics.blocked)}</strong><span>Fails closed before outbound</span></div><div class="ccal-metric"><small>Active rails</small><strong>${safeCount(view.metrics.activeChannels)}</strong><span>Planning variants only</span></div></section>
    <div class="ccal-workspace"><section class="ccal-calendar" aria-labelledby="ccal-calendar-title"><header class="ccal-section-head"><div><h2 id="ccal-calendar-title">${view.filters.mode === 'week' ? 'Weekly signal board' : 'Monthly campaign map'}</h2><p>Channel placements around approved company content. Scroll sideways on compact screens.</p></div><span class="ccal-count">${safeCount(view.visibleSlotCount)} draft slot${view.visibleSlotCount === 1 ? '' : 's'}</span></header><div class="ccal-scroll" tabindex="0" aria-label="Scrollable ${escapeHtml(view.filters.mode)} content calendar"><div class="ccal-weekdays" aria-hidden="true">${weekdays}</div><div class="ccal-grid ${escapeHtml(view.filters.mode)}">${days}</div></div></section>${backlog(view)}</div>
    ${warning}
