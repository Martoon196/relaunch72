...
import { socialImageView } from './social-image-view.js';
...
function exactReviewActions(
  ...
  if (review.social?.artworkInstructions && !review.social.image) {
    return '<section class="pcr-panel"><header class="pcr-subhead"><h2>Add the image first</h2><p>This post has image instructions, but no picture yet. Add and save the image, then review the words and picture together.</p><a class="pcr-action-link" href="#post-image">Add image</a></header></section>';
  }
...
export function renderPortalCompanyContentReviewBody(
  ...
  const imagePanel = socialImageView(snapshot, options);
  const exactCopy = (email
    ...
    : social
      ? `<section class="pcr-panel" aria-labelledby="pcr-social-title"><header class="pcr-panel-head"><h2 id="pcr-social-title">Your post</h2><span class="pcr-channel">${escapeHtml(social.platform)}</span></header><div class="pcr-readable"><div class="pcr-post-text">${escapeHtml(social.publicationCopy)}</div>${social.legacyCombined ? '<p class="pcr-help">This older draft may include image notes in the post text. Use Edit post to separate them before approval.</p>' : ''}${social.artworkInstructions ? `<details class="pcr-notes"><summary>Image or video notes</summary><div class="pcr-post-text">${escapeHtml(social.artworkInstructions)}</div><p class="pcr-help">These notes guide the picture. They will not appear in your published post.</p></details>` : ''}${social.ctaUrl ? `<div class="pcr-cta"><strong>Link in this post</strong><span>${escapeHtml(social.ctaUrl)}</span></div>` : ''}</div></section>`
      : `<section class="pcr-panel"><header class="pcr-panel-head"><h2>Content preview</h2></header><div class="pcr-readable"><div class="pcr-post-text">${escapeHtml(review.canonicalContent)}</div></div></section>`) + imagePanel;
  const planAction = social && review.isLatest && !review.approvalStale && review.approvalStatus === 'approved'
    ? `<section class="pcr-panel"><header class="pcr-subhead"><h2>Next: open your calendar</h2><p>Your post is approved. See your saved plan or choose an account and time. You do not need to approve it again.</p></header><div class="pcr-readable"><a class="pcr-action-link" href="${CONTENT_CALENDAR_ROUTE}?content_version=${encodeURIComponent(review.contentVersionId)}#saved-plans-title">Open calendar →</a></div></section>` : '';
