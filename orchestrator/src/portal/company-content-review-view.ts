...
function socialRevisionActions(
  ...
  return `<details class="pcr-panel" id="edit-version"><summary class="pcr-panel-head"><span><strong>Correct this draft</strong><small style="display:block;margin-top:5px;color:var(--pcr-muted)">Saving creates version ${review.versionNumber + 1}; it never changes this version.</small></span><span class="pcr-chip warn">New immutable version</span></summary><form class="pcr-readable" method="post" action="${CONTENT_SOCIAL_REVISION_ROUTE}"><input type="hidden" name="_csrf" value="${escapeHtml(security.csrfToken)}"><input type="hidden" name="command_key" value="${escapeHtml(security.revisionCommandKey)}"><input type="hidden" name="content_item_id" value="${escapeHtml(review.contentItemId)}"><input type="hidden" name="previous_version_id" value="${escapeHtml(review.contentVersionId)}"><input type="hidden" name="expected_content_sha256" value="${escapeHtml(review.contentSha256)}"><input type="hidden" name="exact_revision_token" value="${escapeHtml(security.exactRevisionToken)}"><input type="hidden" name="return_exact_item_id" value="${escapeHtml(review.contentItemId)}"><input type="hidden" name="return_exact_version_id" value="${escapeHtml(review.contentVersionId)}"><label class="pcr-readable-label" for="pcr-publication-copy">Publication copy</label><textarea class="pcr-body" style="width:100%" id="pcr-publication-copy" name="publication_copy" maxlength="900000" required>${escapeHtml(social.publicationCopy)}</textarea><label class="pcr-readable-label" style="margin-top:22px" for="pcr-artwork-instructions">Artwork instructions</label><textarea class="pcr-body" style="width:100%;min-height:150px" id="pcr-artwork-instructions" name="artwork_instructions" maxlength="50000" placeholder="Describe the image or video separately from the words people will read.">${escapeHtml(social.artworkInstructions ?? '')}</textarea><button class="pcr-action-link" style="margin-top:14px" type="submit">Save as a new version</button><p style="margin:10px 0 0;color:var(--pcr-muted);font-size:11px">The new version starts unapproved. Nothing is posted or scheduled.</p></form></details>`;
}
...
export function renderPortalCompanyContentReviewBody(
  ...
  const exactCopy = email
    ...
    : social
      ? `<section class="pcr-panel" aria-labelledby="pcr-social-title"><header class="pcr-panel-head"><div><h2 id="pcr-social-title">Social post</h2><p>Readable publication copy and production notes from this exact immutable version.</p></div><span class="pcr-chip good">${escapeHtml(social.platform)}</span></header><div class="pcr-readable"><span class="pcr-readable-label">Publication copy</span><pre class="pcr-body" tabindex="0"><code>${escapeHtml(social.publicationCopy)}</code></pre><div class="pcr-cta"><strong>Copy SHA-256</strong><code>${escapeHtml(social.publicationCopySha256)}</code></div><span class="pcr-readable-label" style="margin-top:22px">Artwork instructions</span>${social.artworkInstructions ? `<pre class="pcr-body" style="min-height:150px" tabindex="0"><code>${escapeHtml(social.artworkInstructions)}</code></pre><div class="pcr-cta"><strong>Artwork instructions SHA-256</strong><code>${escapeHtml(social.artworkInstructionsSha256)}</code></div>` : `<div class="pcr-cta"><strong>${social.legacyCombined ? 'Legacy combined draft' : 'No artwork instructions supplied'}</strong><span>${social.legacyCombined ? 'This older version may contain art direction inside the publication copy. Save a corrected version to separate the fields.' : 'The exact version contains publication copy only.'}</span></div>`}${social.ctaUrl ? `<div class="pcr-cta"><strong>Destination</strong><span>${escapeHtml(social.ctaUrl)}</span></div>` : ''}</div></section>`
  ...
  const canonicalEvidence = `<details class="pcr-panel pcr-canonical"><summary>Technical evidence <span>${escapeHtml(bytes(review.canonicalByteLength))}</span></summary><pre class="pcr-copy" tabindex="0"><code>${escapeHtml(review.canonicalContent)}</code></pre></details>`;
  const planAction = social && review.isLatest && !review.approvalStale
    && review.approvalStatus === 'approved'
    ? `<section class="pcr-panel"><header class="pcr-subhead"><h2>Ready for planning</h2><p>Carry this exact approved version into the existing calendar. Scheduling remains a separate deliberate step.</p></header><div class="pcr-readable"><a class="pcr-action-link" href="${CONTENT_CALENDAR_ROUTE}?content_version=${encodeURIComponent(review.contentVersionId)}#new-plan">Plan this exact version</a></div></section>` : '';
  return `<style data-property-predator-company-content-review>${STYLE}</style><article class="pcr" aria-labelledby="pcr-ledger-title"><header class="pcr-top"><a class="pcr-back" href="/portal/content"><span aria-hidden="true">←</span> Back to Content Control</a><div class="pcr-lockup"><span class="pcr-mark" aria-hidden="true"><b>PP</b></span><span>Property Predator · Growth HQ</span></div></header>${exactReviewNotice(options.notice)}<section class="pcr-hero"><div><div class="pcr-kicker">Immutable company-content ledger · version ${review.versionNumber.toLocaleString('en-GB')}</div><h1 id="pcr-ledger-title">Review the work.<br><em>Then decide.</em></h1><p class="pcr-lead"><strong>${escapeHtml(review.title)}</strong> is shown as people will use it. The technical evidence stays available in one place below.</p></div><aside class="pcr-status" data-state="${review.approvalStatus === 'rejected' ? 'quarantined' : 'pending'}"><small>Exact-version status</small><strong>${escapeHtml(review.approvalStatus.replace('_', ' '))}</strong><span>${review.isLatest ? 'Latest immutable version.' : 'Historical version; a newer version exists.'} ${review.approvalStale ? 'Any recorded approval is stale.' : 'No stale approval is presented.'}</span></aside></section><section class="pcr-truth"><strong>Review boundary active</strong><p>Approval covers the words and production notes shown here. Planning and delivery remain separate controls.</p><span class="pcr-effects">Review page · no direct call</span></section><div class="pcr-layout"><div class="pcr-main">${exactCopy}${socialRevisionActions(snapshot, options)}${canonicalEvidence}${exactReviewActions(snapshot, options)}${planAction}${ownedSeedWorkflowActions(snapshot, options)}</div><aside class="pcr-side"><section class="pcr-panel"><header class="pcr-subhead"><h2>Exact identity &amp; proof</h2><p>All approval actions target this item and version explicitly.</p></header><div class="pcr-proof-grid">${proof('Content item', review.contentItemId, true)}${proof('Content version', review.contentVersionId, true)}${proof('Content SHA-256', review.contentSha256, true)}${proof('Brand SHA-256', review.brandSha256, true)}${proof('Blob SHA-256', review.blobSha256, true)}${proof('Source system', review.source.system)}${proof('Source item', review.source.itemId)}${proof('Source version', review.source.version)}${proof('MIME type', review.contentMimeType, true)}</div></section><section class="pcr-panel"><header class="pcr-subhead"><h2>Approval target</h2><p>Request and decision identities are immutable evidence, not provider permission.</p></header><div class="pcr-proof-grid"><div class="pcr-proof"><strong>Status</strong><span class="pcr-chip ${stateClass}">${escapeHtml(review.approvalStatus)}</span></div>${proof('Request ID', review.approvalRequestId ?? 'Not requested', true)}${proof('Decision ID', review.approvalDecisionId ?? 'No decision', true)}</div></section></aside></div><footer class="pcr-footer"><span><strong>${escapeHtml(snapshot.workspace.workspaceName)}</strong> · snapshot ${time(snapshot.workspace.snapshotAt)}</span><span>Exact review · deliberate human decision · explicit LIVE staging boundary</span></footer></article>`;
}
