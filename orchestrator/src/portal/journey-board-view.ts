...
const JOURNEY_BOARD_STYLE = `
  ...
  .jb-card{position:relative;border:1px solid var(--jb-line);border-left:3px solid var(--jb-teal);border-radius:8px;background:var(--jb-raised);overflow:hidden}.jb-card[data-band="burning"]{border-left-color:var(--jb-danger)}.jb-card[data-band="hot"]{border-left-color:#ff9f43}.jb-card[data-band="warm"]{border-left-color:var(--jb-amber)}.jb-card[data-dragging="true"]{opacity:.58}.jb-card[data-pointer-target-label]::after{content:"Move to " attr(data-pointer-target-label);position:absolute;z-index:5;inset:8px;display:grid;place-items:center;border:1px solid var(--jb-teal);border-radius:7px;background:rgba(3,17,15,.94);color:var(--jb-teal);font:900 13px var(--mono,monospace);text-align:center;pointer-events:none}.jb-card-head{display:grid;grid-template-columns:minmax(0,1fr) auto;gap:9px;padding:11px 11px 9px}.jb-identity{min-width:0}.jb-person{display:inline-block;color:var(--jb-ink);font-size:14px;font-weight:820;line-height:1.3}.jb-person:hover,.jb-person:focus-visible{color:var(--jb-teal)}.jb-company{display:block;margin-top:3px;color:var(--jb-muted);font-size:12px;overflow:hidden;text-overflow:ellipsis;white-space:nowrap}.jb-score{min-width:50px;border:1px solid var(--jb-line-strong);padding:6px;text-align:center}.jb-score strong{display:block;font:900 18px/1 var(--mono,monospace)}.jb-score span{display:block;margin-top:4px;color:var(--jb-muted);font:800 12px/1 var(--mono,monospace);text-transform:uppercase}.jb-card[data-band="burning"] .jb-score strong,.jb-card[data-band="burning"] .jb-score span{color:var(--jb-danger)}.jb-card[data-band="hot"] .jb-score strong,.jb-card[data-band="hot"] .jb-score span{color:#ff9f43}.jb-card[data-band="warm"] .jb-score strong,.jb-card[data-band="warm"] .jb-score span{color:var(--jb-amber)}
...
function cardMarkup(
  ...
  const movable = view.workspace.canWrite && card.move && validMutation(view.csrfToken, card.move)
    && lanes.some((lane) => lane.id !== card.laneId && card.move!.allowedLaneIds.includes(lane.id));
  return `<li><article class="jb-card" data-journey-card${movable ? ' data-workflow-movable="true"' : ''} data-card-id="${escapeHtml(card.id)}" data-lane-id="${escapeHtml(card.laneId)}" data-band="${escapeHtml(card.scoreBand)}" aria-labelledby="jb-card-${cardIndex}-name">
