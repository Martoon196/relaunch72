...
import type { LiveChannelsPauseScope } from './live-channels-service.js';
...
const STYLE = `
  ...
  .plc-dispatch{padding:13px 18px;border-bottom:1px solid var(--plc-line)}.plc-dispatch-grid{display:grid;grid-template-columns:repeat(6,minmax(0,1fr));gap:5px}.plc-metric{min-width:0;border:1px solid var(--plc-line);background:#0d1113;padding:8px}.plc-metric span{display:block;color:var(--plc-faint);font:750 8px var(--mono,monospace);letter-spacing:.04em;text-transform:uppercase}.plc-metric strong{display:block;margin-top:4px;font:900 16px var(--mono,monospace)}.plc-metric.attention strong{color:var(--plc-amber)}.plc-dispatch-meta{margin:8px 0 0;color:var(--plc-faint);font:700 9px var(--mono,monospace)}
  ...
  .plc-approvals-grid{display:grid;gap:0}.plc-approval-row{display:grid;grid-template-columns:minmax(0,1.2fr) auto minmax(0,1.4fr) auto;gap:12px;align-items:center;padding:12px 18px;border-bottom:1px solid var(--plc-line)}.plc-approval-row:last-child{border-bottom:0}.plc-approval-row h3{margin:0;font-size:13px}.plc-approval-row strong{font:900 17px var(--mono,monospace)}.plc-approval-row strong.zero{color:var(--plc-faint)}.plc-approval-row p{margin:0;color:var(--plc-muted);font-size:11px}.plc-approval-row a{border:1px solid var(--plc-line-2);padding:0 12px;min-height:44px;display:inline-flex;align-items:center;color:var(--plc-teal);font:800 10px var(--mono,monospace);text-decoration:none;white-space:nowrap}.plc-approval-row a:hover{border-color:var(--plc-teal)}
  ...
  @media(max-width:760px){.plc-hero{grid-template-columns:1fr;padding:27px 20px 22px}.plc-boundary{grid-template-columns:1fr;padding:13px 20px}.plc-boundary span{white-space:normal}.plc-notice{grid-template-columns:1fr;padding:12px 20px}.plc-pulse{grid-template-columns:repeat(2,minmax(0,1fr))}.plc-master{grid-template-columns:1fr;padding:15px 20px}.plc-grid{padding:10px}.plc-panel{margin:0 10px 10px}.plc-dispatch-grid{grid-template-columns:repeat(3,minmax(0,1fr))}.plc-approval-row{grid-template-columns:minmax(0,1fr) auto;row-gap:6px}.plc-approval-row p{grid-column:1/-1}.plc-approval-row a{grid-column:1/-1;justify-self:start}.plc-timeline-list li{grid-template-columns:auto minmax(0,1fr);row-gap:4px}.plc-timeline-copy{grid-column:1/-1}.plc-footer{display:grid}}
  @media(max-width:480px){.plc-hero h1{font-size:2.4rem}.plc-launch-split{grid-template-columns:repeat(2,minmax(0,1fr))}.plc-card-head,.plc-next{grid-template-columns:1fr}.plc-state{justify-self:start}.plc-next a{justify-self:start}.plc-gauges,.plc-dispatch-grid{grid-template-columns:1fr}.plc-gov-item{grid-template-columns:1fr;row-gap:4px}.plc-receipt{grid-template-columns:1fr}.plc-card-foot{flex-direction:column}}
...
export interface LiveChannelsRenderOptions {
  ...
  /** True only when the composed service exposes the pause command seam. */
  readonly pauseCommandAvailable: boolean;
  /** Fresh idempotency keys minted by the router for this render, one per pause scope. */
...
function renderPauseGuard(
  ...
  const heading = `<summary><span>Emergency pause</span><b>${card.pauseLabel}</b></summary>`;
  ...
  if (!options.pauseCommandAvailable) {
    return `<details class="plc-guard">${heading}<div class="plc-guard-body"><p>This rail is not paused. The portal pause command seam is not composed yet, so the pause is controlled by the channel's environment switches only.</p><button class="plc-guard-button" type="button" disabled aria-disabled="true">Engage emergency pause — command seam not composed</button></div></details>`;
  }
  return `<details class="plc-guard">${heading}<div class="plc-guard-body">
    ...
    <form method="post" action="${LIVE_CHANNELS_PAUSE_ROUTE}">
      ...
      <input type="hidden" name="command_key" value="${escapeHtml(options.pauseCommandKeys[card.channel])}">
      <input type="hidden" name="scope" value="${escapeHtml(card.channel)}">
...
function renderWhy(card: LiveChannelCardView): string {
  const items = card.whyBlocked.length
    ? card.whyBlocked.map((blocker) => `<li><span>${escapeHtml(blocker.message)}<code>${escapeHtml(blocker.code)}${blocker.derived ? ' · derived from switch state' : ''}</code></span></li>`).join('')
    : `<li class="clear"><span>No blocker is recorded. ${card.pauseEngaged ? 'Only the emergency pause holds this rail dark.' : 'The rail is operating inside its proven envelope.'}</span></li>`;
...
function renderCard(card: LiveChannelCardView, options: LiveChannelsRenderOptions): string {
  const metrics = [
    ['Queued', card.dispatch.queued],
    ['In flight', card.dispatch.inFlight],
    ['Awaiting proof', card.dispatch.awaitingProof],
    ['Attention', card.dispatch.needsAttention],
    ['OK today', card.dispatch.succeededToday],
    ['Failed today', card.dispatch.failedToday],
  ].map(([label, value]) => `<div class="plc-metric${label === 'Attention' && Number(value) > 0 ? ' attention' : ''}"><span>${escapeHtml(String(label))}</span><strong>${escapeHtml(String(value))}</strong></div>`).join('');
  const receipt = card.latestReceipt
    ? `<div class="plc-receipt-copy"><b>${escapeHtml(card.latestReceipt.eventLabel)}</b> · <code>${escapeHtml(card.latestReceipt.safeCode)}</code></div><time datetime="${escapeHtml(card.latestReceipt.occurredAt)}">${escapeHtml(readableInstant(card.latestReceipt.occurredAt))}</time>`
    : '<div class="plc-receipt-copy">No receipt recorded yet. The first dispatch or signed webhook event will appear here as an immutable receipt.</div>';
  const approvalsLine = card.approvalsPending === 0
    ? 'Nothing is waiting on approval for this channel.'
    : `<b>${card.approvalsPending}</b> waiting · oldest ${escapeHtml(readableInstant(card.approvalsOldestAt))}`;
  return `<article class="plc-card ${tone(card.postureTone)}" id="${escapeHtml(card.anchorId)}" aria-labelledby="${escapeHtml(card.anchorId)}-title">
    <header class="plc-card-head"><div><span class="plc-eyebrow">${escapeHtml(card.eyebrow)}</span><h2 id="${escapeHtml(card.anchorId)}-title">${escapeHtml(card.label)}</h2><p class="plc-idline"><b>${escapeHtml(card.providerLabel)}</b> · ${escapeHtml(card.accountLabel)}<br>${escapeHtml(card.connectionLabel)} · ${escapeHtml(card.connectionStatusLabel)}</p></div><span class="plc-state ${tone(card.postureTone)}">${escapeHtml(card.postureLabel)}</span></header>
    <ul class="plc-switchrow" aria-label="Execution and effect switches"><li class="plc-switch">${escapeHtml(card.modeLabel)}</li><li class="plc-switch ${card.effectsLabel === 'EFFECTS ON' ? 'on' : 'off'}">${escapeHtml(card.effectsLabel)}</li>${card.deliveryLabel ? `<li class="plc-switch ${card.deliveryLabel === 'DELIVERY ON' ? 'on' : 'off'}">${escapeHtml(card.deliveryLabel)}</li>` : ''}<li class="plc-switch ${card.pauseEngaged ? 'pause' : 'off'}">PAUSE ${card.pauseLabel}</li><li class="plc-switch">${escapeHtml(card.workerLabel.toUpperCase())}</li></ul>
    ${renderWhy(card)}
    ${(() => {
      const nextLink = allowedLink(card.nextAction.link, options);
      return `<div class="plc-next"><span>Safe next action</span><div class="plc-next-copy"><strong>${escapeHtml(card.nextAction.label)}</strong><small>${escapeHtml(card.nextAction.detail)}</small></div>${nextLink ? `<a href="${escapeHtml(nextLink.href)}">${escapeHtml(nextLink.label)}</a>` : ''}</div>`;
    })()}
    <section class="plc-dispatch" aria-label="Dispatch state"><h3>Dispatch</h3><div class="plc-dispatch-grid">${metrics}</div><p class="plc-dispatch-meta">${escapeHtml(card.dispatch.observedLabel)}</p></section>
    <section class="plc-caps" aria-label="Caps and usage"><h3>Hard caps · usage</h3><div class="plc-gauges">${card.gauges.map(renderGauge).join('')}</div><p class="plc-perjob">${escapeHtml(card.perJobLabel)}</p></section>
    <section class="plc-gov" aria-label="Approvals, consent and scope"><h3>Governance</h3><div class="plc-gov-grid">
      <div class="plc-gov-item"><span>Approvals</span><div class="plc-gov-copy">${approvalsLine}<br>${escapeHtml(card.approvalRequirement)}</div></div>
      <div class="plc-gov-item"><span>Consent</span><div class="plc-gov-copy"><span class="plc-chip ${tone(card.permissionTone)}">${escapeHtml(card.permissionStateLabel)}</span>${escapeHtml(card.permissionDetail)}</div></div>
    ...
    <footer class="plc-card-foot"><span>Contract · ${escapeHtml(card.contract)}</span></footer>
  </article>`;
}

function renderHandoffCard(view: LiveChannelsView): string {
  const rows = [
    [view.handoff.conversionInboxComposed, view.handoff.conversionInboxComposed
    ...
    [view.handoff.lead360Composed, view.handoff.lead360Composed
      ? 'Lead 360 is composed — every contact opens a full case file.'
      : 'Lead 360 is not composed for this workspace yet.'],
    [view.handoff.whatsappProjectionLabel.startsWith('Inbound WhatsApp projects'), view.handoff.whatsappProjectionLabel],
    [false, view.handoff.inboundLastDayLabel],
  ...
  return `<article class="plc-card muted" id="live-conversion-handoff" aria-labelledby="live-conversion-handoff-title">
    <header class="plc-card-head"><div><span class="plc-eyebrow">Conversion handoff</span><h2 id="live-conversion-handoff-title">Inbox &amp; Lead 360</h2><p class="plc-idline">Where every live reply, receipt and consent change becomes CRM truth.</p></div><span class="plc-state ${view.handoff.conversionInboxComposed ? 'ready' : 'muted'}">${view.handoff.conversionInboxComposed ? 'Connected' : 'Not composed'}</span></header>
...
function renderMasterStop(view: LiveChannelsView, options: LiveChannelsRenderOptions): string {
  if (view.allPaused) {
    return `<section class="plc-master" aria-label="Master emergency stop"><div class="plc-master-copy"><strong>Master emergency stop · engaged everywhere</strong><span>Every live rail already holds an engaged emergency pause. Releasing any pause is a separate founder decision made outside this portal.</span></div><span class="plc-chip">ALL RAILS PAUSED</span></section>`;
  }
  const body = options.pauseCommandAvailable
    ? `<details class="plc-guard"><summary><span>Pause every live rail</span><b>ARMED</b></summary><div class="plc-guard-body">
        <p>This engages the emergency pause on customer email, owned social and Meta WhatsApp at once. Nothing can be released from this portal afterwards.</p>
        <form method="post" action="${LIVE_CHANNELS_PAUSE_ROUTE}">
          <input type="hidden" name="_csrf" value="${escapeHtml(options.csrfToken)}">
    ...
    : `<details class="plc-guard"><summary><span>Pause every live rail</span><b>UNAVAILABLE</b></summary><div class="plc-guard-body"><p>The portal pause command seam is not composed, so the master stop is controlled by environment switches only.</p><button class="plc-guard-button" type="button" disabled aria-disabled="true">Engage master emergency stop — command seam not composed</button></div></details>`;
  return `<section class="plc-master" aria-label="Master emergency stop"><div class="plc-master-copy"><strong>Master emergency stop</strong><span>At least one rail is not paused. The master stop is fail-safe: it can only move rails towards OFF.</span></div>${body}</section>`;
}

function renderApprovalsPanel(view: LiveChannelsView): string {
  if (view.totalApprovalsPending === 0) {
    return `<section class="plc-panel" aria-labelledby="plc-approvals-title"><div class="plc-panel-head"><h2 id="plc-approvals-title">Approval queue</h2><span>0 waiting</span></div><div class="plc-empty"><b>Nothing is waiting on approval.</b>Every live dispatch requires an explicit human approval first — when one is requested it appears here and in the Conversion Inbox approval queue.</div></section>`;
  }
  const rows = view.channels.map((card) => `<div class="plc-approval-row">
      <h3>${escapeHtml(card.label)}</h3>
      <strong${card.approvalsPending === 0 ? ' class="zero"' : ''}>${card.approvalsPending}</strong>
      <p>${card.approvalsPending === 0 ? 'Nothing waiting.' : `Oldest waiting since ${escapeHtml(readableInstant(card.approvalsOldestAt))}. ${escapeHtml(card.approvalRequirement)}`}</p>
      <a href="${CONVERSION_INBOX_ROUTE}?queue=approval">Review queue</a>
    </div>`).join('');
  return `<section class="plc-panel" aria-labelledby="plc-approvals-title"><div class="plc-panel-head"><h2 id="plc-approvals-title">Approval queue</h2><span>${view.totalApprovalsPending} waiting</span></div><div class="plc-approvals-grid">${rows}</div></section>`;
}

function renderTimelinePanel(view: LiveChannelsView): string {
  const body = view.timeline.length
    ? `<ol class="plc-timeline-list">${view.timeline.map((event) => `<li><time datetime="${escapeHtml(event.occurredAt)}">${escapeHtml(readableInstant(event.occurredAt))}</time><span class="plc-timeline-chip">${escapeHtml(event.channelLabel)}</span><span class="plc-timeline-copy">${escapeHtml(event.eventLabel)} · <code>${escapeHtml(event.safeCode)}</code></span><span class="plc-chip ${tone(event.tone)}">${escapeHtml(event.eventKind)}</span></li>`).join('')}</ol>`
    : '<div class="plc-empty"><b>No receipts recorded yet.</b>When a dispatch is accepted or a signed provider webhook lands, its immutable receipt appears here newest-first. Nothing is inferred; only recorded receipts are shown.</div>';
  return `<section class="plc-panel" aria-labelledby="plc-timeline-title"><div class="plc-panel-head"><h2 id="plc-timeline-title">Receipt timeline</h2><span>${view.timeline.length} recorded</span></div>${body}</section>`;
}
...
export function renderLiveChannelsBody(
  ...
  const boundary = view.illustrative
    ? '<strong>ILLUSTRATIVE TEST DATA</strong><p>Every identity, count, receipt and instant on this page is invented to demonstrate the operating view. Nothing was read from Mailgun, Ayrshare, Meta or any database, and nothing here can authorise a send.</p>'
    : '<strong>OBSERVED EVIDENCE</strong><p>Bounded read-only evidence from the live channel foundations. This page cannot load a credential, flip an effect switch on, or create a provider operation.</p>';
  ...
  return `${renderContentWorkspaceNavigation('live', { companyAssetsAvailable: true, composerAvailable: true, brandBrainAvailable: true, liveChannelsAvailable: true, providerReadinessAvailable: options.railStatusAvailable })}<style>${STYLE}</style><section class="plc" aria-labelledby="live-channels-title" data-dataset="${escapeHtml(view.dataset)}">
    <header class="plc-hero"><div class="plc-hero-copy"><span class="plc-kicker">PropertyPredator · live operations</span><h1 id="live-channels-title">Every channel.<br><em>One operational truth.</em></h1><p>Customer email, owned social and Meta WhatsApp on one fail-closed control room. Every state below is proven by switch tuples, hard caps and immutable receipts — never assumed, never decorative.</p></div><aside class="plc-launch" aria-label="Launch readiness"><div class="plc-launch-top"><span>Launch readiness</span><span class="plc-chip ${tone(view.launchReadinessTone)}">${escapeHtml(view.allPaused ? 'PAUSED' : view.readyCount === view.channels.length ? 'ALL LIVE' : view.readyCount > 0 ? 'PARTIAL' : 'NOT LIVE')}</span></div><h2>${escapeHtml(view.launchReadinessLabel)}</h2><ul class="plc-launch-split">${splits.map(([toneClass, label, count]) => `<li class="${toneClass}"><span>${label}</span><strong>${count}</strong></li>`).join('')}</ul><p>${view.allPaused ? 'The emergency pause is engaged on every rail. No provider call can begin anywhere.' : 'Rails move one controlled gate at a time. A channel only counts as live when its full switch tuple proves it.'}</p></aside></header>
    <aside class="plc-boundary" role="status">${boundary}<span>${view.illustrative ? 'NO PROVIDER WAS READ' : 'READ-ONLY EVIDENCE'}</span></aside>
    ${notice}
    <section class="plc-pulse" aria-label="Operations summary"><div class="plc-pulse-lead"><span>Today across all rails</span><strong>${view.totalUsedToday} of ${view.totalDailyCap} capped dispatches used</strong><p>Hard caps are enforced twice — at enqueue and again at call time — per workspace and connection, in UTC.</p></div><div class="plc-pulse-stat"><span>Queued</span><strong>${view.totalQueued}</strong><small>waiting on gates</small></div><div class="plc-pulse-stat${view.totalNeedsAttention > 0 ? ' attention' : ''}"><span>Needs attention</span><strong>${view.totalNeedsAttention}</strong><small>human decision required</small></div><div class="plc-pulse-stat${view.totalApprovalsPending > 0 ? ' attention' : ''}"><span>Approvals waiting</span><strong>${view.totalApprovalsPending}</strong><small>before anything sends</small></div><div class="plc-pulse-stat"><span>Receipts</span><strong>${view.timeline.length}</strong><small>recorded in timeline</small></div></section>
    ${renderMasterStop(view, options)}
    <div class="plc-grid">${view.channels.map((card) => renderCard(card, options)).join('')}${renderHandoffCard(view)}</div>
    ${renderApprovalsPanel(view)}
    ${renderTimelinePanel(view)}
    <footer class="plc-footer"><span><strong>Snapshot</strong> · ${escapeHtml(readableInstant(view.snapshotAt))} · ${escapeHtml(view.workspaceName)}</span><span><strong>Safety invariant</strong> · read-only evidence · no credentials · pause commands only move rails towards OFF</span></footer>
  </section>`;
}
