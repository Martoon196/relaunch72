...
import {
  ...
  writeOwnedPublicSocialWorkerFailure,
} from './runner.js';
...
async function main(): Promise<void> {
  ...
  const shutdown = (): void => {
    void runtime.shutdown().catch((error: unknown) => {
      ...
      writeOwnedPublicSocialWorkerFailure('shutdown', shutdownFailureCount, error);
      process.exitCode = 1;
    });
  };
...
main().catch((error: unknown) => {
  writeOwnedPublicSocialWorkerFailure('startup', 1, error);
  process.exitCode = 1;
});
