-- Forward-extend the exact Zernio engagement boundary to Facebook Page DMs
-- and owned-post comments. Existing Instagram and LinkedIn evidence remains
-- immutable; only the allowed network set and network-qualified functions grow.
...
ALTER TABLE app.property_predator_zernio_reply_drafts
  ...
  CHECK (network IN ('facebook', 'instagram', 'linkedin')) NOT VALID;
ALTER TABLE app.property_predator_zernio_reply_drafts
...
CREATE OR REPLACE FUNCTION app_private.zernio_reply_channel_truth(
...
LANGUAGE sql STABLE SECURITY DEFINER SET search_path = pg_catalog AS $function$
  ...
  WHERE session_user = 'r72_zernio_social_command'
    ...
    AND p_network IN ('facebook', 'instagram', 'linkedin')
    AND octet_length(p_provider_profile_id_sha256) = 32
    AND octet_length(p_provider_account_id_sha256) = 32
$function$;

CREATE OR REPLACE FUNCTION app_private.create_zernio_reply_draft(
...
CREATE OR REPLACE FUNCTION app_private.claim_zernio_reply_send(
  p_workspace_id uuid, p_provider_connection_id uuid, p_draft_id uuid,
